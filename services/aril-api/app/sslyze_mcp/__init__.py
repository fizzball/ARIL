"""Self-contained SSLyze TLS/cert MCP server (Streamable HTTP, JSON-RPC).

Runs as a managed subprocess launched by the ARIL macOS app. Reuses the
gateway's already-bundled FastAPI/uvicorn stack (no extra dependencies) so it
freezes into the same PyInstaller binary. Wraps the local `sslyze` CLI and
exposes it to MCP clients over an authenticated localhost endpoint.
"""

from app.sslyze_mcp.config import SSLyzeMCPConfig
from app.sslyze_mcp.server import build_app, run

__all__ = ["SSLyzeMCPConfig", "build_app", "run"]
